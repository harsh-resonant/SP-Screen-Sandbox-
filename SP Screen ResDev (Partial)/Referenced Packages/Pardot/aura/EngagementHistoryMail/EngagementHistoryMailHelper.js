({
	loadForPeople : function(component, people) {
		var emails = this.getEmailAddressesFromPeople(people);
	...
	getEmailAddressesFromPeople: function(people) {
		var records = [];
	...
	setVersionDependencies: function(cmp) {
		if (cmp && cmp.isValid()) {
			if ($A.storageService && $A.storageService.version === "41.0") {
				var override = cmp.find('override');
				$A.util.addClass(override, 'two-ten-compat');
			}
		}
	}
})
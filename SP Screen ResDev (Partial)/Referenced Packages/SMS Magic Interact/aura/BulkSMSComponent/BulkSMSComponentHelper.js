({
    retRecipientList : function(component, event, helper, ids) {
        ...
        action.setCallback(this, function(response){
            ...
            if (state === "SUCCESS") {
                ...
                component.set("v.AllowToSchedule", recipientClass.AllowToSchedule);
                component.set("v.smsHistoryPrefix", recipientClass.smsHistoryPrefix);
                component.set("v.objectLabel", recipientClass.objectLabel);
    ...
    doSendSMS : function(component, event, helper, ids) {
        var str = "";
        if(!helper.checkAllParamters(component, helper)){
            return;
        }
        ...
        action.setCallback(this, function(response){
            ...
            if (state === "SUCCESS") {
                var returnVal = response.getReturnValue();
                if(returnVal.name === "201") {
                    var successDiv = component.find("successMessage");
                    var sendSuccessMessage = component.find("sendSuccessMessage");
                    var BulkSMSDiv = component.find("BulkSMSDiv");
                    helper.showDiv(successDiv);
                    helper.showDiv(sendSuccessMessage);
                    helper.hideDiv(BulkSMSDiv);
    ...
    scheduleMessage : function(component, helper){
        ...
        var scheduleObject = JSON.stringify(component.get("v.scheduleObject"));
        var action = component.get("c.scheduleBulkSMS");
        action.setParams({"recordIdList" 		: trimedIds,
                          "objectName"	 		: objectName,
                          "mobilePhoneFields"	: mobileFields,
                          "senderId"			: senderId,
        ...
        action.setCallback(this, function(response){
            ...
            if (state === "SUCCESS") {
                var returnVal = response.getReturnValue();
                if(returnVal.name === "201") {
                    component.set("v.scheduledId", returnVal.label);
                    var successDiv = component.find("successMessage");
                    var scheduleSuccessMessage = component.find("scheduleSuccessMessage");
                    var BulkSMSDiv = component.find("BulkSMSDiv");
                    helper.showDiv(successDiv);
                    helper.showDiv(scheduleSuccessMessage);
                    helper.hideDiv(BulkSMSDiv);
                    component.set("v.auraInfoMessage", "");
                    component.set("v.auraSuccessMessage", "");
                    component.set("v.auraErrorMessage", "");
    ...
    checkAllParamters : function(component, helper){
        var objectName = component.get("v.objectName");
        var str;
    ...
    openScheduleModal : function (component){
        var scheduleModal = component.find("scheduleModal");
    ...
    deleteSchedule : function (component, event, helper){
    	var schedulerIdList = [];
        var scheduleId = component.get("v.scheduledId");
        schedulerIdList.push(scheduleId);
        var action = component.get("c.deleteScheduleJob");
        action.setParams({"recordIds" 	: schedulerIdList});
        action.setCallback(this, function(response){
            var state = response.getState();
            if (state === "SUCCESS") {
                var returnVal = response.getReturnValue();
                if(returnVal.name === "201") {
                	helper.closeNotificationModal(component);
    				helper.navigationToHome(component, event, helper);
                } else if(returnVal.name === "500") {
                    helper.hideDiv(component.find("notificationModalFooter"));
                    helper.showDiv(component.find("notificationModalbody"));
                    component.set("v.notificationModalText", returnVal.ErrorMessage);
                }
            }
        });
        $A.enqueueAction(action);
    },
    closeNotificationModal : function (component){
        var notificationModal = component.find("notificationModal");

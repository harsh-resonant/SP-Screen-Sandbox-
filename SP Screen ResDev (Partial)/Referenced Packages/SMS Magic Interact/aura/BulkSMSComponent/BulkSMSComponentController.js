...
/*        if(selected.length > 700) {
    ...
    onScheduleButtonPress : function (component, event, helper) {
        if(!component.get('v.isScheduling')){
    		helper.openScheduleModal(component);
    	} else {
    		helper.scheduleMessage(component, helper);
		}
    },
    openScheduleModal : function (component, event, helper) {
    	helper.openScheduleModal(component);
    },
    scheduleModalEvent : function (component, event) {
        if(event){
            var scheduleObject = event.getParam('scheduleObject');
            var scheduleMessage = event.getParam('scheduleMessage');
            var scheduledMessage = event.getParam('scheduledMessage');
            component.set('v.scheduleMessage', scheduleMessage);
            component.set('v.scheduleObject', scheduleObject);
            component.set('v.scheduledMessage', scheduledMessage);
            component.set("v.auraErrorMessage", "");
            component.set('v.isScheduling', true);
        }
    },
    cancelSchedule : function (component) {
        component.set('v.scheduleMessage', '');
        component.set('v.scheduleObject', {});
        component.set('v.isScheduling', false);
        var scheduleModal = component.find("scheduleModal");
        scheduleModal.resetSchedule();
    },
    navigateToSMSHistory : function (component){
        var smsHistoryPrefix = component.get("v.smsHistoryPrefix");
        /*global sforce:true*/
        if(typeof sforce != 'undefined' && sforce.one) {
        	sforce.one.navigateToURL('/'+smsHistoryPrefix);
        } else {
            window.location.href = '/'+smsHistoryPrefix;
        }
    },
    openDeleteConfModal : function (component, event, helper){
    	helper.showDiv(component.find("notificationModalFooter"));
        helper.hideDiv(component.find("notificationModalbody"));
    	var notificationModal = component.find("notificationModal");
        notificationModal.openModal();
    },
    cancelScheduleDetele : function (component, event, helper){

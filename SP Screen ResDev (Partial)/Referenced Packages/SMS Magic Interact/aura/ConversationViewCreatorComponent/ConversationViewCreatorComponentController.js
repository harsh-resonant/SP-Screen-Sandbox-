({
    ...
    createChat : function(component) {
        ...
        if(isEmpty(ObjectId) || isEmpty(pageAPIName) || isEmpty(pageLabelName)) {
            component.set("v.auraMessage", $A.get("$Label.smagicinteract.CONV_VIEW_ERR_MSG"));
            return;
        }
        ...
        postHTTPCallout.setCallback(this, function(data) {
            ...
            if(returnVal.name === "201") {
                component.set("v.successMessage", $A.get("$Label.smagicinteract.Conversation_View_Success_Message"));
            } else {
